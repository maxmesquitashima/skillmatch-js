...
const listaVagas = [
  new VagaFrontEnd("TechStart", "Desenvolvedor Front-End Júnior", ["JavaScript", "HTML", "CSS", "React", "Git", "Lógica de Programação", "Inglês Básico"], 3500.0, "Remoto", "Júnior"),
  new VagaFrontEnd("CodeLab", "Estagiário Front-End", ["HTML", "CSS", "JavaScript", "Lógica de Programação", "GitHub"], 2000.0, "Híbrido", "Estágio"),
  new VagaFrontEnd("WebSolutions", "Desenvolvedor Front-End Júnior", ["JavaScript", "React", "Redux", "TypeScript", "Git", "Metodologias Ágeis", "Comunicação"], 4000.0, "Presencial", "Júnior")
];
...
async function iniciarSistema() {
  ...
  try {
    ...
    for (const vaga of vagas) {
      ...
      const habilidadesAtendidas = candidato.habilidades.filter((habilidadeCandidato) =>
        vaga.requisitos.includes(habilidadeCandidato)
      );
      ...
      console.log(`Habilidades do candidato que atendem: ${habilidadesAtendidas.join(", ")}`);
      console.log(`Percentual de Compatibilidade: ${percentualCompatibilidade.toFixed(2)}%`);
      ...
      console.log(`Habilidades Faltantes para esta vaga: ${habilidadesFaltantes.join(", ")}`);

      habilidadesFaltantes.forEach((habilidade) => todasHabilidadesFaltantes.add(habilidade));
    ...
    if (melhorCompatibilidade.vaga) {
      ...
      console.log(`A vaga com maior compatibilidade é: ${melhorCompatibilidade.vaga.cargo} na ${melhorCompatibilidade.vaga.empresa}`);
      console.log(`Com um percentual de: ${melhorCompatibilidade.percentual.toFixed(2)}%`);
    }

    // [RF07] Recomendação Textual de Estudos
    console.log("\n--- Recomendação de Estudos ---");
    if (todasHabilidadesFaltantes.size > 0) {
      console.log(`Para aumentar suas chances, considere focar nos seguintes estudos: ${Array.from(todasHabilidadesFaltantes).join(", ")}.`);
    } else {
    ...
    finalizarAnalise(candidato.nome, (nome) => {
      console.log(`Olá ${nome}, sua análise de compatibilidade foi concluída com sucesso!`);
    });
